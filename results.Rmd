...
barplot(bar_data, names.arg = tourer)
```
...
```{r results='asis'}
kept = !dat$Avalanche_Size_RP%in%c('R3.5','R5')
chisq.test(table(dat$Avalanche_Size_RP[kept], dat$Tourer[kept]))
contin_table<-table(dat$Avalanche_Size_RP[kept], dat$Tourer[kept])
xtable(contin_table)
ggplot(data = data.frame(contin_table), aes(x = Var1, y = Freq, fill = Var2))+ geom_bar(stat = 'identity',position = position_dodge())
```
