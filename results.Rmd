...
get_plot('Preparedness4')
get_plot('Avalanche_Size_RP')
get_plot('Avalanche_Size_DF')
```
...
ggplot(data = dat, aes(x = Tourer, y = Total_in_party))+
...
xtable(contin_table)
ggplot(data = data.frame(contin_table), aes(x = Var1, y = Freq, fill = Var2))+ geom_bar(stat = 'identity',position = position_dodge())
```
...
xtable(contin_table)
ggplot(data = data.frame(contin_table), aes(x = y, y = Freq, fill = x))+ geom_bar(stat = 'identity',position = position_dodge())+labs(x = 'Avalanche Size (Relative)', y = 'Frequency', fill = 'Tourer')+
  labs(x = 'Avalanche Size (Relative)', y = 'Frequency', fill = 'Tourer' )
...
contin_table<-table(y,x)
xtable(contin_table)
ggplot(data = data.frame(contin_table), aes(x = y, y = Freq, fill = x))+ geom_bar(stat = 'identity',position = position_dodge())+
...
ggplot(data = dat, aes(x = Tourer, y = Burial_Depth))+
  geom_boxplot()+
  stat_summary(fun.y = mean, geom = 'point', shape = 8, size = 4)+
